...
class Config(object):
    ...
    def _core(self):
        self.interactive_add('core', 'nick', 'Introduce el apodo para tu bot',
                             'UnivBot')
        self.interactive_add('core', 'host', 'Ingrese al servidor para conectarse',
                             'irc.dftba.net')
        self.add_option('core', 'use_ssl', 'Si el bot se conecta con SSL')
        if self.use_ssl == 'True':
            default_port = '6697'
        else:
            default_port = '6667'
        self.interactive_add('core', 'port', 'Ingrese el puerto para conectarse',
                             default_port)
        self.interactive_add(
            'core', 'owner',
            "Ingrese su propio nombre de IRC (o el del propietario del bot)"
        )
        c = 'Ingrese los canales para conectarse de forma predeterminada, uno a la vez.' + \
            ' Cuando termines, pulsa enter otra vez.'
